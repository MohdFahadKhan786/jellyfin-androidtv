...
public class LegacyImageCardView extends BaseCardView {
    ...
    public LegacyImageCardView(Context context, boolean showInfo) {
        ...
        // ddflix: Netflix-style focus animation. Leanback's default BaseCardView has no
        // built-in scale/lift on focus (only the dim-when-unfocused behavior via
        // overlayDimDimmedLevel) — this is genuinely new behavior.
        setOnFocusChangeListener((focusedView, hasFocus) -> {
            focusedView.animate()
                    .scaleX(hasFocus ? 1.08f : 1f)
                    .scaleY(hasFocus ? 1.08f : 1f)
                    .translationZ(hasFocus ? Utils.convertDpToPixel(getContext(), 6) : 0)
                    .setDuration(150)
                    .start();
        });
    }
